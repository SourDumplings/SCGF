...
int parse_scgf_xml()
{
	const std::string filePath = "res/dist/scgf/design/100000000.img.scgf";

	try
	{
		// 读取文件
		auto [fileData, fileDataSize] = scgf::util::read_file(filePath.c_str());

		// 解密
		auto [decryptedData, decryptedSize] =
			scgf::util::Cryptor::get_instance().decrypt_scgf(fileData.get(), fileDataSize);
