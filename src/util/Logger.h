...
#define SCGF_LOG_TRACE(...) \
	SPDLOG_LOGGER_TRACE(scgf::util::Logger::get_instance().get_logic_logger(), __VA_ARGS__)
#define SCGF_LOG_DEBUG(...) \
	SPDLOG_LOGGER_DEBUG(scgf::util::Logger::get_instance().get_logic_logger(), __VA_ARGS__)
#define SCGF_LOG_INFO(...) \
	SPDLOG_LOGGER_INFO(scgf::util::Logger::get_instance().get_logic_logger(), __VA_ARGS__)
#define SCGF_LOG_WARN(...) \
	SPDLOG_LOGGER_WARN(scgf::util::Logger::get_instance().get_logic_logger(), __VA_ARGS__)

// Error log will abort the program after logging in DEBUG mode!
#ifdef DEBUG
#define SCGF_LOG_ERROR(...)                                                                  \
	SPDLOG_LOGGER_ERROR(scgf::util::Logger::get_instance().get_logic_logger(), __VA_ARGS__); \
...
#define SCGF_LOG_CRITICAL(...)                                                                  \
	SPDLOG_LOGGER_CRITICAL(scgf::util::Logger::get_instance().get_logic_logger(), __VA_ARGS__); \
	std::exit(EXIT_FAILURE)

// TODO use proto generate stat log msg
#define SCGF_STAT(logMsg) \
	SPDLOG_LOGGER_INFO(scgf::util::Logger::get_instance().get_stat_logger(), logMsg)
...
private:
#ifdef PROJECT_NAME
	const char* LOGIC_LOG_FILE_PATH = DIST_DIR "/log/" PROJECT_NAME ".log";
	const char* STAT_LOG_FILE_PATH	= DIST_DIR "/log/" PROJECT_NAME ".stat";
#else
	const char* LOGIC_LOG_FILE_PATH = DIST_DIR "/log/scgf.log";
	const char* STAT_LOG_FILE_PATH	= DIST_DIR "/log/scgf.stat";
#endif
