...
namespace scgf::util
{
std::pair<std::unique_ptr<char[]>, size_t> Compressor::compress(const char* data, size_t dataSize)
{
	z_stream zs;
...
std::pair<std::unique_ptr<char[]>, size_t>
	Compressor::decompress(const char* compressedData, size_t compressedDataSize)
{
	z_stream zs;
	memset(&zs, 0, sizeof(zs));
