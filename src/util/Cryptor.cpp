...
namespace scgf::util
{
void Cryptor::xor_buf(const BYTE in[], BYTE out[], size_t len) const
{
	for (size_t idx = 0; idx < len; ++idx)
		out[idx] ^= in[idx];
}

int Cryptor::aes_decrypt_cbc(
	const BYTE	   in[],
	size_t		   in_len,
	BYTE		   out[],
	const AES_WORD key[],
...
std::pair<std::unique_ptr<char[]>, size_t> Cryptor::decrypt_scgf(const char* data, size_t dataSize)
	const
{
