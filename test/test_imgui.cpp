...
int test_imgui()
{
	...
	std::filesystem::path imguiConfPath = cwd / std::filesystem::path("../conf/dist/imgui.ini");
	io.IniFilename						= NULL;
	
	ImGui::LoadIniSettingsFromDisk(imguiConfPath.string().c_str());
	...
	std::filesystem::path fontPath = cwd / std::filesystem::path("../res/dist/font/simfang.ttf");
...
#endif
	{
		...
		// named window.
		{
			...
			ImGui::Text("This is some useful text. 这是一些有用的信息。"
